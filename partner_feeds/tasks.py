...
def update_posts_for_feed_task(partner):
    ...
    for entry in feed.entries:
        ...
        try:
            ...
            # try to get the date of the entry from either 'date' or 'published',
            # otherwise, try the date of the feed
            try:
                ...
                entry_date = time.strftime("%Y-%m-%d %H:%M:%S", entry_date)
                ...
                try:
                    ...
                    entry_date = timelib.strtotime(
                        feed.date)  # convert to a timestamp
                    entry_date = time.localtime(entry_date)  # converts to a time.struct_time (with regards to local timezone)
